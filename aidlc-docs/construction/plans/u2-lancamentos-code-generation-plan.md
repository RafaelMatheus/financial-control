...
- [ ] **Passo 1** — `build.gradle.kts`: adicionar **ArchUnit** (`com.tngtech.archunit:archunit-junit5`)
      como dependência de teste — *D-66*
- [ ] **Passo 2** — `common/web/Erros.kt`: acrescentar os códigos que as 23 regras exigem —
...
- [ ] **Passo 3** — `dominio/`: `Categoria` puro (sem JPA), com a bicondicional escopo↔grupo na
      construção, e a porta `CategoriaRepositorio` estendendo `RepositorioComVisibilidade` —
      *RN-C01, RN-C03, D-63*
- [ ] **Passo 4** — `aplicacao/CategoriaService`: criar, renomear, excluir (com realocação
      transacional), listar (com o conjunto inicial sob demanda) — *RN-C02, C04 a C08*
- [ ] **Passo 5** — `adaptador/persistencia/`: `CategoriaJpa`, Spring Data, adaptador aplicando o
      critério, mapeadores. **`saveAndFlush`**, pela lição do defeito 1 de `cd310cb`
- [ ] **Passo 6** — `adaptador/web/`: `CategoriaController` e DTOs com Bean Validation.
      **Sem `@Email`-style de validação que duplique regra de domínio**, pela lição do defeito 2
- [ ] **Passo 7** — Testes de `categoria`: unidade do serviço; integração com Testcontainers para
...
- [ ] **Passo 8** — `dominio/`: `Gasto` puro com as 5 invariantes; `FiltroGasto` com `de`/`ate`
...
- [ ] **Passo 9** — `aplicacao/GastoService`: lançar, editar, excluir, consultar, totalizar.
      `lancar` **não recebe `donoId`** — a regra fica na assinatura — *RN-L05*
- [ ] **Passo 10** — `adaptador/persistencia/`: `GastoJpa` (com `cartao_id` e `competencia`
...
- [ ] **Passo 11** — `adaptador/web/`: `GastoController` com `GET /gastos` (paginado, teto 100) e
      `GET /gastos/totais` — *D-57*
- [ ] **Passo 12** — Testes de `gasto`: unidade do serviço; integração para valor não positivo,
...
- [ ] **Passo 13** — `V2__lancamentos.sql`: tabelas `categoria` e `gasto`; **dois índices únicos
      ...
      *RNF-04, D-01*
- [ ] **Passo 14** — 🔒 **Teste de isolamento de gastos** — o mais importante da unidade. Dois
...
- [ ] **Passo 15** — Teste de concorrência: dois cadastros simultâneos da mesma categoria no mesmo
      grupo, verificando que o índice parcial barra o segundo

### Os dois totais

- [ ] **Passo 16** — 🔬 **Testes de propriedade dos totais** com Kotest: `totalPessoal` contém
...
- [ ] **Passo 17** — Teste de que **nenhum DTO de saída** contém a soma dos dois totais — *RN-T04*
- [ ] **Passo 18** — Teste de propriedade da bicondicional de visibilidade: para qualquer par
...
- [ ] **Passo 19** — 🔒 **`ArquiteturaTest`** (D-66): reprova o build quando uma entidade com campo
...
- [ ] **Passo 20** — Rodar o `ArquiteturaTest` **contra o código de U1** e registrar o resultado.
      Se ele reprovar algo já entregue, o achado vale mais que a regra — analisar antes de ajustar
      qualquer um dos dois

### Fechamento

- [ ] **Passo 21** — Compilar e rodar a suíte local (a que não depende de Docker)
- [ ] **Passo 22** — Empurrar e **esperar o CI verde**. A lição do Passo 26 de U1 é explícita: teste
      escrito e não executado é documentação, não verificação. **Esta stage não se declara concluída
      antes do CI passar**
- [ ] **Passo 23** — Verificação final: nenhum `Double`/`Float` em caminho monetário; nenhuma
...
- [ ] **Passo 24** — Resumo em `aidlc-docs/construction/u2-lancamentos/code/code-summary.md`
...
*Preenchido durante a execução. Um plano que só registra sucesso não serve para auditar nada.*
